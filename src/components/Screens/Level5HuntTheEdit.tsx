...
type GameScreenState = 'welcome' | 'instructions' | 'round' | 'roundComplete' | 'timesUp' | 'final';

const BEST_SCORE_KEY = 'indianEditBestScore';
const BEST_TIME_KEY = 'indianEditBestTime';

export const Level5HuntTheEdit: React.FC = () => {
  const { state: globalState, navigateTo, updateHuntScore, finishHuntGame, toggleSound } = useGame();
  ...
  useEffect(() => {
    ...
    timerIntervalRef.current = setInterval(() => {
      setTimeRemaining((prev) => {
        ...
        if (next === 10 || next === 5 || next === 3 || next === 2 || next === 1) {
          sound.playWarning();
        }
  ...
  const checkAndSaveBestScore = useCallback((newScore: number, roundTimeTaken: number) => {
    ...
    setBestRoundTime((prevTime) => {
      ...
      const topTime = prevTime > 0 ? Math.min(prevTime, validRoundTime) : validRoundTime;
      try {
  ...
  // Handle Bottle Found
  const handleBottleFound = useCallback((bottle: HiddenBottle) => {
    ...
    if (updatedFound.length >= currentRound.bottles.length) {
      ...
      const speedBonus = Math.max(0, Math.floor(timeRemaining * SCORING.TIME_BONUS_PER_SEC));
  ...
  return (
    <div className="relative min-h-screen bg-[#070302] text-[#faf5eb] flex flex-col font-sans overflow-x-hidden">
      ...
      {screenState === 'final' && (
        <FinalResult
          ...
          onContinueToGrandFinale={() => {
            finishHuntGame(score > 0 ? score : 27850, 15, bestRoundTime || 18.6);
