...
import { Sparkles, Trophy, ArrowRight, Share2, Award, Wine, Compass, Gift } from 'lucide-react';
...
export const ResultScreen: React.FC = () => {
  ...
  const scoreRows = [
    { label: 'Harvest Rush (L1)', score: state.scoreRush, max: 2000, icon: '🍊' },
    { label: 'Zero Mile Precision (L2)', score: state.scoreZero, max: 1000, icon: '📍' },
    { label: 'Bottle Provenance (L3)', score: state.decodeScore, max: 1000, icon: '🔍' },
    { label: 'Alchemy & Blend (L4)', score: state.blendScore, max: 2000, icon: '🥃' },
    { label: 'Nagpur Vision (L5)', score: state.scoreCity, max: 3500, icon: '🏛️' },
  ];
    ...
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 animate-fade-in">
      ...
      <div className="gold-card p-6 sm:p-10 text-center relative overflow-hidden">
        ...
        <p className="mt-2 text-sm sm:text-base text-[#f5d77f] font-serif italic max-w-xl mx-auto">
          "{personality?.tagline || 'Where heritage meets contemporary refinement.'}"
        </p>

        <p className="mt-4 text-xs sm:text-sm text-[#warm-beige] max-w-2xl mx-auto leading-relaxed">
          {personality?.description || 'Your refined palate and design eye celebrate authentic Indian heritage, blending timeless craft with forward-looking ambition.'}
        </p>
        ...
        <div className="mt-8 p-4 rounded-2xl bg-[#170f0a]/90 border border-[#d4af37]/50 max-w-md mx-auto flex items-center justify-around shadow-inner">
          <div className="text-center">
            <span className="text-[10px] text-[#a69383] uppercase tracking-wider font-bold">TOTAL MASTER SCORE</span>
            <div className="font-serif text-3xl font-bold gold-gradient-text tabular-nums mt-0.5">
              {state.totalScore > 0 ? state.totalScore.toLocaleString() : (state.scoreRush + state.scoreZero + state.decodeScore + state.blendScore + state.scoreCity).toLocaleString()}
            </div>
          </div>
          <div className="h-10 w-px bg-[#3d261a]" />
          <div className="text-center">
            <span className="text-[10px] text-[#a69383] uppercase tracking-wider font-bold">VIP STANDING</span>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-5 gap-2.5 text-left max-w-3xl mx-auto">
          {scoreRows.map((row, idx) => (
            <div key={idx} className="p-3 rounded-xl bg-[#22160f] border border-[#3d261a]">
              <div className="flex items-center justify-between text-[11px] mb-1">
                <span>{row.icon}</span>
                <span className="font-mono font-bold text-[#f5d77f]">{row.score}</span>
              </div>
              <div className="text-[10px] text-[#a69383] truncate">{row.label}</div>
        ...
        {personality?.signatureCocktail && (
          <div className="mt-8 p-5 rounded-2xl bg-gradient-to-r from-[#2e1e15] to-[#24150e] border border-[#d4af37]/40 max-w-xl mx-auto text-left flex items-start gap-4">
            ...
            <div>
              <span className="text-[10px] font-bold tracking-widest text-[#d4af37] uppercase">Curated Pour</span>
