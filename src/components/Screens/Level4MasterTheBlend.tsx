...
export const Level4MasterTheBlend: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto py-6 px-4 sm:px-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#2e1e15] border border-[#d4af37]/40 text-xs text-[#f5d77f]">
            <span>LEVEL 04 / 05</span>
            <span>•</span>
            <span className="text-[#a69383]">ALCHEMIC CRAFT & SPEED</span>
          </div>
          <h2 className="font-serif text-2xl sm:text-3xl font-bold text-[#faf6f0] mt-1">
            Master The Blend
          </h2>
          <p className="text-xs text-[#a69383] mt-1">
            Harmonize Nagpur orange botanicals, oak barrel depth, and peated
            smoke to craft the signature edit.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() =>
              setViewMode(viewMode === "iframe" ? "native" : "iframe")
            }
            className="text-xs px-3 py-2 rounded-xl bg-[#2e1e15] border border-[#d4af37]/40 text-[#f5d77f] hover:bg-[#3d261a] transition-all"
          >
            {viewMode === "iframe"
              ? "Switch to Studio Mode"
              : "Switch to 3D Canvas Mode"}
          </button>
        </div>
      </div>
      ...
      {/* Main Game Stage */}
      {viewMode === "iframe" ? (
        <div className="gold-card p-4 overflow-hidden relative">
          <div className="w-full h-155 rounded-xl overflow-hidden bg-[#0a0a0c] relative border border-[#3d261a]">
            <iframe
              ref={iframeRef}
            ...
            <div className="text-xs text-[#warm-beige]">
              {state.blendCompleted ? (
                <span className="text-green-400 font-semibold flex items-center gap-1.5">
                  ...
                  <span>
                    Blend mastered! Score recorded: {state.blendScore} pts.
                  </span>
                </span>
              ) : (
                <span>
                  Complete the blend challenge in the frame above to record your
                  score.
                </span>
              )}
            </div>
