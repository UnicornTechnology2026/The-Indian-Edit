...
interface BottleHotspotProps {
  ...
  isFound: boolean;
  justFound: boolean;
  onBottleClick: (e: React.MouseEvent | React.TouchEvent, bottle: HiddenBottle) => void;
}
  ...
  return (
    <>
      ...
      >
        ...
        {justFound && (
          <div className="absolute -top-10 left-1/2 -translate-x-1/2 z-30 whitespace-nowrap pointer-events-none animate-bounce flex items-center gap-1 px-3 py-1 rounded-full bg-gradient-to-r from-[#d4af37] to-[#f7e7a9] text-[#070403] font-bold text-xs shadow-[0_4px_20px_rgba(212,175,55,0.9)]">
            <Sparkles className="w-3 h-3 text-[#070403]" />
            <span>FOUND! +1000</span>
          </div>
        )}
      </div>
