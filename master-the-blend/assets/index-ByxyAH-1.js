(() => {
  ...
  function init(){
    ...
    // Guarded: if #bgCanvas isn't present in the DOM (wrong page, ID
    // mismatch, stripped during build, etc.) skip the effect instead of
    // throwing and killing the rest of the script.
    const canvas = document.getElementById('bgCanvas');
    if (!canvas) {
      console.warn('[indian-edit] #bgCanvas not found in the DOM — skipping background particle effect. Check that <canvas id="bgCanvas"></canvas> exists on this page and that this script runs on the same page/document.');
    } else {
      ...
      resizeCanvas();
      const colors = ['rgba(230,202,133,', 'rgba(212,175,55,', 'rgba(244,239,224,', 'rgba(230,142,39,'];
    ...
    if (missingScreens.length) {
      console.warn(`[indian-edit] Missing screen element(s) in the DOM: ${missingScreens.join(', ')}. Check that the page markup matches the expected IDs (introScreen, gameScreen, completedScreen, timeoutScreen).`);
    }
    ...
    function renderIngredients(){
      ...
      if (!list) {
        console.warn('[indian-edit] #ingredientsList not found — cannot render ingredient tiles.');
        return;
      }
      list.innerHTML = '';
      order.forEach(ing=>{
    ...
    if (!vessel) {
      console.warn('[indian-edit] #vessel not found in the DOM — drag-and-drop into the bottle will be unavailable. Tap-to-add on ingredients still works if #ingredientsList exists.');
    } else {
    ...
    function updateVessel(){
      ...
      const pctEl = document.getElementById('fillPercent');
      if (pctEl) pctEl.textContent = pct.toFixed(2)+'%';
      const statusEl = document.getElementById('fillStatus');
      if (statusEl) statusEl.textContent = n===0?'EMPTY':n===1?'1/3 FILLED':n===2?'2/3 FILLED':'MASTERED';
